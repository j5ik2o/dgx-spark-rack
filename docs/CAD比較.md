...
追加試験として、[支持棒](FreeCAD支持棒の評価.md)と[左側板](FreeCAD側板の評価.md)を履歴付きで再構築し、寸法変更・復元・保存後の再編集を確認しました。以下は4台構成をSTEPで取り込んだ際の記録です。
...
Fusionとの差が大きいのは、チームでの設計管理や、電子回路・加工・自動設計を統合した作業です。今回のラックで必要な形状作成機能は、支持棒と側板の試験で確認できています。

| 領域 | Fusion | FreeCADでの進め方 |
|---|---|---|
| チームでの設計管理 | クラウドで共有・版管理・アクセス権を管理 | Gitや共有サービスを別途組み合わせる |
| 電子回路と基板 | 回路図、2D配線、3D基板を同じ製品で扱う | KiCadなどを使い、StepUpで機械設計と連携 |
| 自動設計と高度な加工 | 製造条件を考慮した複数案の自動生成、同時5軸加工など | 必要な処理に合わせ、追加ツールや外部CAMを検討 |

Fusion側の根拠は[プロジェクトと権限](https://help.autodesk.com/view/fusion360/ENU/?contextId=FT-PROJECTS)、[ワークスペース](https://help.autodesk.com/view/fusion360/ENU/?contextId=GS-WORKSPACES)、[拡張機能](https://help.autodesk.com/view/fusion360/ENU/index.html?guid=GUID-EEE56836-C03A-4B10-A51C-A6203CF46AC4)です。自動設計・高度な加工などには拡張契約が必要な機能があります。

FreeCADは[組立・FEM・CAMも備えています](https://blog.freecad.org/2026/03/25/freecad-version-1-1-released/)。[KiCad StepUp](https://github.com/easyw/kicadStepUpMod)は電子回路側と機械設計側をつなぐ追加ワークベンチです。FreeCADが拡張を含めても原理的にできないという一覧ではなく、同じ目的を達成するための構成と作業手順の違いとして整理しています。
...
1. **推奨：Fusionを正本に保ち、側板と支持棒から1ユニットを組み立てる。** 単体部品の追加試験は完了したので、部品間の連動と干渉を確認します。
