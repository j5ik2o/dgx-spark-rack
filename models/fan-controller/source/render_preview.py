...
for clamp in n['clamps']:
    draw(ax, clamp, '#dd922b')
...
draw(ax, n['body_final'], '#52758d')
for clamp in n['clamps']:
    draw(ax, clamp, '#dd922b', lift=18)
draw(ax, n['lid_final'], '#dce4eb', lift=35)
ax.set_title('本体・独立押さえ4個・蓋の分解表示', fontsize=15)
fig.text(.05,.12,'青：本体　　灰：蓋　　橙：厚さ3mmの基板押さえ（各1本のねじで固定）', fontsize=12)
...
for clamp in n['clamps']:
    draw(ax, clamp, '#dd922b')
ax.set_title('本体内部：着座ポケットと短い基板押さえ', fontsize=14)
ax = fig.add_subplot(122, projection='3d')
